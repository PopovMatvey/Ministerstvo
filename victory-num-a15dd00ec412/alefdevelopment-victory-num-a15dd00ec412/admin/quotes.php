  ...
  0 => '',
    ...
    $actions[''] = function () {
				...
				<form method="POST">
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<small>Вставьте сюда новые записи. Каждая запись на новой строчке: <b class="csv-create-format">ID, Факт, Время нахождения</b></small>
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Факты".' </h2>
	...
	<div class="genesis-header">
		<div>
			...
			<div class="genesis-header-property">
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=quote&sort_order='. ($next_order['quote']) .'\' class=\'sort\' column=\'quote\' sort_order=\''.$sort_order['quote'].'\'>Факт'. $sort_icon['quote'].'</a>
			</div>

			<div class="genesis-header-property">
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=timeout&sort_order='. ($next_order['timeout']) .'\' class=\'sort\' column=\'timeout\' sort_order=\''.$sort_order['timeout'].'\'>Время нахождения'. $sort_icon['timeout'].'</a>
			</div>
...
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
			<span class='genesis-attached-column-info'>
				<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=quote&sort_order='. ($next_order['quote']) .'\' class=\'sort\' column=\'quote\' sort_order=\''.$sort_order['quote'].'\'>'. (str_replace('style="margin-left:5px;"', '', !empty($sort_icon['quote']) ? $sort_icon['quote'] : '<span class="fa fa-sort"></span>')).'</a>'."</span>
				<span class='genesis-attached-column-name'>Факт:</span>
			</span><span class='editable' data-placeholder='' data-inp='textarea' data-url='engine/ajax.php?action=editable&table=bottom_quotes' data-pk='{$item['id']}' data-name='quote'>".htmlspecialchars($item['quote'])."</span></div>", $item, "Факт"):"<div class='genesis-item-property '>
			<span class='genesis-attached-column-info'>
				<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=quote&sort_order='. ($next_order['quote']) .'\' class=\'sort\' column=\'quote\' sort_order=\''.$sort_order['quote'].'\'>'. (str_replace('style="margin-left:5px;"', '', !empty($sort_icon['quote']) ? $sort_icon['quote'] : '<span class="fa fa-sort"></span>')).'</a>'."</span>
				<span class='genesis-attached-column-name'>Факт:</span>
			</span><span class='editable' data-placeholder='' data-inp='textarea' data-url='engine/ajax.php?action=editable&table=bottom_quotes' data-pk='{$item['id']}' data-name='quote'>".htmlspecialchars($item['quote'])."</span></div>")."
".(function_exists("processTD")?processTD("
		<div class='genesis-item-property '>
			<span class='genesis-attached-column-info'>
				...
				<span class='genesis-attached-column-name'>Время нахождения:</span>
			</span>
			<span class='editable' data-placeholder='В секундах' data-inp='number' data-url='engine/ajax.php?action=editable&table=bottom_quotes' data-pk='{$item['id']}' data-name='timeout'>".htmlspecialchars($item['timeout'])."</span>
		</div>", $item, "Время нахождения"):"
		<div class='genesis-item-property '>
			<span class='genesis-attached-column-info'>
				<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=timeout&sort_order='. ($next_order['timeout']) .'\' class=\'sort\' column=\'timeout\' sort_order=\''.$sort_order['timeout'].'\'>'. (str_replace('style="margin-left:5px;"', '', !empty($sort_icon['timeout']) ? $sort_icon['timeout'] : '<span class="fa fa-sort"></span>')).'</a>'."</span>
				<span class='genesis-attached-column-name'>Время нахождения:</span>
...
EOT;
    ...
    $actions['edit'] = function () {
        ...
        if (isset($id)) {
            $item = q("SELECT * FROM bottom_quotes WHERE id=?", [$id]);
            $item = $item[0];
        }
		            ...
		            <p>
		              '.$item["id"].'
							...
							<div class="form-group">
								<label class="control-label" for="textinput">Факт</label>
								<div>
									<textarea id="quote" name="quote" class="form-control input-md  ckeditor" style="height:500px;" >'.htmlspecialchars($item["quote"]).'</textarea>
								</div>
							</div>
								...
								<div class="form-group">
									<label class="control-label" for="textinput">Время нахождения</label>
									<div>
										<input id="timeout" name="timeout" type="number" placeholder="В секундах" class="form-control input-md "  value="'.htmlspecialchars($item["timeout"]).'">
									</div>
								</div>
    ...
    $actions['edit_page'] = function () {
        ...
        $html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Факты".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
    ...
    $actions['reorder'] = function () {
        ...
        for ($i=0; $i < count($line); $i++) {
            qi("UPDATE `bottom_quotes` SET `` = ? WHERE id = ?", [$i, $line[$i]]);
        }
    ...
    $actions['change_group'] = function () {
        ...
        qi("UPDATE `bottom_quotes` SET `` = ? WHERE id = ?", [$group_id, $id]);
    ...
    $actions['csv_create_execute'] = function () {
        ...
        $sql = "INSERT IGNORE INTO bottom_quotes (`quote`, `timeout`) VALUES (?, ?)";
    ...
    $actions['create_execute'] = function () {
        ...
        $params = [$quote, $timeout];
        $sql = "INSERT INTO bottom_quotes (`quote`, `timeout`) VALUES (?, ?)";
    ...
    $actions['edit_execute'] = function () {
        ...
        if (!$skip) {
            ...
            $set[] = is_null($_REQUEST['quote'])?"`quote`=NULL":"`quote`='".addslashes($_REQUEST['quote'])."'";
            $set[] = is_null($_REQUEST['timeout'])?"`timeout`=NULL":"`timeout`='".addslashes($_REQUEST['timeout'])."'";

            if (count($set)>0) {
                $set = implode(", ", $set);
                $sql = "UPDATE bottom_quotes SET $set WHERE id=?";
    ...
    $actions['delete'] = function () {
        ...
        $id = $_REQUEST['id'];
        try {
            qi("DELETE FROM bottom_quotes WHERE id=?", [$id]);
    ...
    function get_agregate($group_id = "")
    {
        ...
        $sql = "SELECT 1 as stub  FROM (SELECT main_table.*  FROM bottom_quotes main_table) temp $srch $filter $where $group_where $order";
    ...
    function get_data($force_kill_pagination=false)
    {
        ...
        $debug = (isset($_REQUEST['alef_debug']) && $_REQUEST['alef_debug']==1);
        if ($pagination == 1) {
            $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM bottom_quotes main_table) temp $srch $filter $where $order LIMIT :start, :limit";
            ...
            $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM bottom_quotes main_table) temp $srch $filter $where $order";
            if (in_array($_SERVER['SERVER_NAME'], ["test-genesis.alef.im", "devtest-genesis.alef.im", "localhost"]) || $debug) {
                if ($_REQUEST['action']=='') {
                    echo "<!--SQL DATA {$sql} -->";
                }
    ...
    $finalPage = masterRender("Факты", $content, 6, "genesis-body-presentation-table genesis-grouping-style-");
