<?php
  ...
  0 => '',
    ...
    $actions[''] = function () {
        $country_id_values = json_encode(q("SELECT name as text, id as value FROM countries", []));
        $country_id_values_text = "";
        foreach (json_decode($country_id_values, true) as $opt) {
            $country_id_values_text.="<option value=\"{$opt['value']}\">{$opt['text']}</option>";
        }
        ...
        $next_order['id']='asc';
        $next_order['name']='asc';
        $next_order['country_id']='asc';
        $next_order['country_number']='asc';
				...
				<form method="POST">
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<small>Вставьте сюда новые записи. Каждая запись на новой строчке: <b class="csv-create-format">ID, Название, Страна, ID области на карте</b></small>
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Области".' </h2>
	...
	<div class="genesis-header">
		<div>
			...
			<div class="genesis-header-property">
				<nobr>
					<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=country_id&sort_order='. ($next_order['country_id']) .'\' class=\'sort\' column=\'country_id\' sort_order=\''.$sort_order['country_id'].'\'>Страна'. $sort_icon['country_id'].'</a>
					
			<span class=\'fa fa-filter filter btn btn-default\' data-placement=\'bottom\' data-content=\'<div class="input-group">
							<select class="form-control filter-select" name="country_id_filter">


							'.str_replace(chr(39), '&#39;', $country_id_values_text).'
			...
			<div class="genesis-header-property">
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=country_number&sort_order='. ($next_order['country_number']) .'\' class=\'sort\' column=\'country_number\' sort_order=\''.$sort_order['country_number'].'\'>ID области на карте'. $sort_icon['country_number'].'</a>
			</div>
...
".(function_exists("processTD")?processTD("
	<div class='genesis-item-property '>
		...
		<span class='editable' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=regions' data-pk='{$item['id']}' data-name='name'>".htmlspecialchars($item['name'])."</span>
	</div>", $item, "Название"):"
	<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=name&sort_order='. ($next_order['name']) .'\' class=\'sort\' column=\'name\' sort_order=\''.$sort_order['name'].'\'>'. (str_replace('style="margin-left:5px;"', '', !empty($sort_icon['name']) ? $sort_icon['name'] : '<span class="fa fa-sort"></span>')).'</a>'."</span>
...
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
				<span class='genesis-attached-column-info'>
					<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=country_id&sort_order='. ($next_order['country_id']) .'\' class=\'sort\' column=\'country_id\' sort_order=\''.$sort_order['country_id'].'\'>'. (str_replace('style="margin-left:5px;"', '', !empty($sort_icon['country_id']) ? $sort_icon['country_id'] : '<span class="fa fa-sort"></span>')).'</a>'."
			<span class='fa fa-filter filter ' data-placement='bottom' data-content='<div class=\"input-group\">
					...
					<span class='genesis-attached-column-name'>Страна:</span>
				</span><span class='editable' data-inp='select' data-type='select' data-source='".htmlspecialchars($country_id_values, ENT_QUOTES, 'UTF-8')."' data-url='engine/ajax.php?action=editable&table=regions' data-pk='{$item['id']}' data-name='country_id'>".select_mapping($country_id_values, $item['country_id'])."</span></div>", $item, "Страна"):"<div class='genesis-item-property '>
				<span class='genesis-attached-column-info'>
					<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=country_id&sort_order='. ($next_order['country_id']) .'\' class=\'sort\' column=\'country_id\' sort_order=\''.$sort_order['country_id'].'\'>'. (str_replace('style="margin-left:5px;"', '', !empty($sort_icon['country_id']) ? $sort_icon['country_id'] : '<span class="fa fa-sort"></span>')).'</a>'."
			<span class='fa fa-filter filter ' data-placement='bottom' data-content='<div class=\"input-group\">
							<select class=\"form-control filter-select\" name=\"country_id_filter\">
					...
					<span class='genesis-attached-column-name'>Страна:</span>
				</span><span class='editable' data-inp='select' data-type='select' data-source='".htmlspecialchars($country_id_values, ENT_QUOTES, 'UTF-8')."' data-url='engine/ajax.php?action=editable&table=regions' data-pk='{$item['id']}' data-name='country_id'>".select_mapping($country_id_values, $item['country_id'])."</span></div>")."
".(function_exists("processTD")?processTD("
		<div class='genesis-item-property '>
			<span class='genesis-attached-column-info'>
				<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=country_number&sort_order='. ($next_order['country_number']) .'\' class=\'sort\' column=\'country_number\' sort_order=\''.$sort_order['country_number'].'\'>'. (str_replace('style="margin-left:5px;"', '', !empty($sort_icon['country_number']) ? $sort_icon['country_number'] : '<span class="fa fa-sort"></span>')).'</a>'."</span>
				<span class='genesis-attached-column-name'>ID области на карте:</span>
			</span>
			<span class='editable' data-placeholder='' data-inp='number' data-url='engine/ajax.php?action=editable&table=regions' data-pk='{$item['id']}' data-name='country_number'>".htmlspecialchars($item['country_number'])."</span>
		</div>", $item, "ID области на карте"):"
		<div class='genesis-item-property '>
			<span class='genesis-attached-column-info'>
				<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=country_number&sort_order='. ($next_order['country_number']) .'\' class=\'sort\' column=\'country_number\' sort_order=\''.$sort_order['country_number'].'\'>'. (str_replace('style="margin-left:5px;"', '', !empty($sort_icon['country_number']) ? $sort_icon['country_number'] : '<span class="fa fa-sort"></span>')).'</a>'."</span>
				<span class='genesis-attached-column-name'>ID области на карте:</span>
...
EOT;
    ...
    $actions['edit'] = function () {
        ...
        if (isset($id)) {
            $item = q("SELECT * FROM regions WHERE id=?", [$id]);
            $item = $item[0];
        }

        
        $country_id_options = q("SELECT name as text, id as value FROM countries", []);
        $country_id_options_html = "";
        foreach ($country_id_options as $o) {
            $country_id_options_html .= "<option value=\"{$o['value']}\" ".($o["value"]==$item["country_id"]?"selected":"").">{$o['text']}</option>";
        }
        ...
        $html = '
			<form class="form" enctype="multipart/form-data" method="POST">
				...
				<div class="form-group">
					<label class="control-label" for="textinput">Страна</label>
					<div>
						<select id="country_id" name="country_id" class="form-control input-md " >
							'.$country_id_options_html.'
							</select>
					</div>
				</div>
								...
								<div class="form-group">
									<label class="control-label" for="textinput">ID области на карте</label>
									<div>
										<input id="country_number" name="country_number" type="number" placeholder="" class="form-control input-md "  value="'.htmlspecialchars($item["country_number"]).'">
									</div>
								</div>
    ...
    $actions['edit_page'] = function () {
        ...
        $html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Области".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
    ...
    $actions['reorder'] = function () {
        ...
        for ($i=0; $i < count($line); $i++) {
            qi("UPDATE `regions` SET `` = ? WHERE id = ?", [$i, $line[$i]]);
        }
    ...
    $actions['change_group'] = function () {
        ...
        qi("UPDATE `regions` SET `` = ? WHERE id = ?", [$group_id, $id]);
    ...
    $actions['csv_create_execute'] = function () {
        ...
        $sql = "INSERT IGNORE INTO regions (`name`, `country_id`, `country_number`) VALUES (?, ?, ?)";
    ...
    $actions['create_execute'] = function () {
        ...
        $country_number = $_REQUEST['country_number'];

        $params = [$name, $country_id, $country_number];
        $sql = "INSERT INTO regions (`name`, `country_id`, `country_number`) VALUES (?, ?, ?)";
    ...
    $actions['edit_execute'] = function () {
        ...
        if (!$skip) {
            ...
            $set[] = is_null($_REQUEST['country_id'])?"`country_id`=NULL":"`country_id`='".addslashes($_REQUEST['country_id'])."'";
            $set[] = is_null($_REQUEST['country_number'])?"`country_number`=NULL":"`country_number`='".addslashes($_REQUEST['country_number'])."'";

            if (count($set)>0) {
                $set = implode(", ", $set);
                $sql = "UPDATE regions SET $set WHERE id=?";
    ...
    $actions['delete'] = function () {
        ...
        $id = $_REQUEST['id'];
        try {
            qi("DELETE FROM regions WHERE id=?", [$id]);
    ...
    function filter_query($srch)
    {
        ...
        if (isset2($_REQUEST['country_id_filter'])) {
            $filters[] = "`country_id` = '{$_REQUEST['country_id_filter']}'";
        }
    ...
    function filter_divs()
    {
        ...
        $text_option = array_filter(json_decode($country_id_values, true), function ($i) {
            return $i['value']==$_REQUEST['country_id_filter'];
        });
        $text_option = array_values($text_option)[0]['text'];
        if (isset2($_REQUEST['country_id_filter'])) {
            $filter_divs .= "
			<div class='filter-tag'>
					<input type='hidden' class='filter' name='country_id_filter' value='{$_REQUEST['country_id_filter']}'>
					<span class='fa fa-times remove-tag'></span> Страна: <b>{$text_option}</b>
			</div>";
    ...
    function get_agregate($group_id = "")
    {
        ...
        $sql = "SELECT 1 as stub  FROM (SELECT main_table.*  FROM regions main_table) temp $srch $filter $where $group_where $order";
    ...
    function get_data($force_kill_pagination=false)
    {
        ...
        $debug = (isset($_REQUEST['alef_debug']) && $_REQUEST['alef_debug']==1);
        if ($pagination == 1) {
            $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM regions main_table) temp $srch $filter $where $order LIMIT :start, :limit";
            ...
            $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM regions main_table) temp $srch $filter $where $order";
            if (in_array($_SERVER['SERVER_NAME'], ["test-genesis.alef.im", "devtest-genesis.alef.im", "localhost"]) || $debug) {
                if ($_REQUEST['action']=='') {
                    echo "<!--SQL DATA {$sql} -->";
                }
    ...
    $finalPage = masterRender("Области", $content, 3, "genesis-body-presentation-table genesis-grouping-style-");
