        ...
        if (isset($_GET['code'])) {
            ...
            if ($result == true) {
                ...
                if (!isset($user['id']) || $user['id']=='') {
                    ...
                    qi("INSERT INTO `{$mysql_user_table}` (".implode(', ', array_keys($fields)).") VALUES (".implode(', ', array_map(function ($i) {
                        return "?";
                    ...
                    qi("UPDATE `{$mysql_user_table}` SET ".implode(", ", array_map(function ($k) {
                        return "`{$k}`=?";
                    }, array_keys($params)))." WHERE id = ?", array_values(array_merge($params, [$user_id])));
