...
function renderAuth() // окошко авторизации
{
    ...
    if (trim($auth_bg)=="") {
        $auth_bg = "//genesis2.alef.im/default_bg_for_auth.png";
    }

    if (trim($auth_bg_block)=="") {
        $auth_bg_block = "//genesis2.alef.im/default_bg_for_auth.png";
    }
...
function masterRender($title, $content, $activeMenuItem, $bodyExtraClass = "")
{
    include __DIR__."/../menu.php";
...
<html lang="ru">
  <head>
	...
	{$extra_head}
			...
			function cropImages()
			{
				$('.cropped-image:not(.already-cropped)').each(function()
			  {
			  	$(this).addClass('already-cropped');
			    var canvas = $('<canvas></canvas>');
			    var context = canvas[0].getContext('2d');

			    var x1 = $(this).attr('image-x1');
			    var y1 = $(this).attr('image-y1');
			    var x2 = $(this).attr('image-x2');
			    var y2 = $(this).attr('image-y2');
			    this.crossOrigin = '';

			    canvas[0].width = x2-x1;
			    canvas[0].height = y2-y1;
			    canvas[0].style.width = (x2-x1)+'px';
			    canvas[0].style.height = (y2-y1)+'px';
			    context.drawImage(this, x1, y1, x2-x1, y2-y1, 0, 0, x2-x1, y2-y1);
			    this.src = canvas[0].toDataURL("image/png");
	...
	<body class="concept {$bodyExtraClass}">
