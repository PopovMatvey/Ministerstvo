  ...
  0 => '',
    ...
    $actions[''] = function () {
				...
				<form method="POST">
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<small>Вставьте сюда новые записи. Каждая запись на новой строчке: <b class="csv-create-format">Название, Изображение</b></small>
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '.'Графики'.' </h2>
...
<div class="genesis-header-property"></div>
			...
			<div class="genesis-header-property">
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=url&sort_order='.($next_order['url']).'\' class=\'sort\' column=\'url\' sort_order=\''.$sort_order['url'].'\'>Изображение'.$sort_icon['url'].'</a>
			</div>
			<div class="genesis-header-property"></div>
		</div>
</div>
...
'.(function_exists('processTD') ? processTD("
	<div class='genesis-item-property '>
		...
		<span class='editable' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=graphics' data-pk='{$item['id']}' data-name='name'>".htmlspecialchars($item['name']).'</span>
	</div>', $item, 'Название') : "
	<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=name&sort_order='.($next_order['name']).'\' class=\'sort\' column=\'name\' sort_order=\''.$sort_order['name'].'\'>'.(str_replace('style="margin-left:5px;"', '', !empty($sort_icon['name']) ? $sort_icon['name'] : '<span class="fa fa-sort"></span>')).'</a>'."</span>
...
'.(function_exists('processTD') ? processTD("<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=url&sort_order='.($next_order['url']).'\' class=\'sort\' column=\'url\' sort_order=\''.$sort_order['url'].'\'>'.(str_replace('style="margin-left:5px;"', '', !empty($sort_icon['url']) ? $sort_icon['url'] : '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Изображение:</span>
		</span>
					".($item['url'] ? "<a href='#' data-featherlight='{$item['url']}'>" : '')."
						...
						data-file-exists='".(file_exists($_SERVER['DOCUMENT_ROOT'].'/'.($item['url'] ? $item['url'] : 'style/placeholder.jpg')) ? '1' : '0')."'
						src='".($item['url'] ? $item['url'] : 'style/placeholder.jpg')."'
						 image-url='".($item['url'] ? $item['url'] : 'style/placeholder.jpg')."' />
					".($item['url'] ? '</a>' : '').'
...
EOT;
    ...
    $actions['edit'] = function () {
        ...
        if (isset($id)) {
            $item = q('SELECT * FROM graphics WHERE id=?', [$id]);
            $item = $item[0];
        }
                    ...
                    .'
						...
						<div class="form-group form-group-position-relative">
							...
							<div class="">
								...
								data-file-exists="'.((!empty($item['url'])) && (file_exists($_SERVER['DOCUMENT_ROOT'].'/'.$item['url'])) ? '1' : '0').'"
								src="'.$item['url'].'" image-url="'.$item['url'].'" class="genesis-image js-quotable" style="max-width:200px; max-height:200px;" /><label for="url" class="file"> Выберите изображение <input type="file" name="url" id="url" /></label></div>
						</div>
    ...
    $actions['edit_page'] = function () {
        ...
        $html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'.'Графики'.' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
    ...
    $actions['reorder'] = function () {
        ...
        for ($i = 0; $i < count($line); ++$i) {
            qi('UPDATE `graphics` SET `sort` = ? WHERE id = ?', [$i, $line[$i]]);
        }
    ...
    $actions['change_group'] = function () {
        ...
        qi('UPDATE `graphics` SET `` = ? WHERE id = ?', [$group_id, $id]);
    ...
    $actions['csv_create_execute'] = function () {
        ...
        $sql = 'INSERT IGNORE INTO graphics (`name`, `url`) VALUES (?, ?)';
    ...
    $actions['create_execute'] = function () {
        ...
        if (isset($url) && $url['name'] !== '') {
            ...
            $tm = time();
            $ext = '.'.pathinfo($url['name'], PATHINFO_EXTENSION);
            $target_file = $_SERVER['DOCUMENT_ROOT'].'/uploads/'.$tm.'_'.md5($url['name']).$ext;
            if (move_uploaded_file($url['tmp_name'], $target_file)) {
                compressImage($target_file);
                $url = '/uploads/'.$tm.'_'.md5($url['name']).$ext;
            } else {
                $set[] = "`url`=''";
        ...
        $params = [$name, $url];
        $sql = 'INSERT INTO graphics (`name`, `url`) VALUES (?, ?)';
    ...
    $actions['edit_execute'] = function () {
        ...
        if (!$skip) {
            ...
            if (isset($_FILES['url']) && $url['name'] !== '') {
                ...
                if (move_uploaded_file($url['tmp_name'], $target_file)) {
                    compressImage($target_file);
                    $set[] = "`url`='".('/uploads/'.$tm.'_'.md5($url['name'])).$ext."'";
                } else {
                    $set[] = "`url`=''";
                    buildMsg('Не удается загрузить изображение. Попробуйте отправить файл меньшего размера.', 'danger');
                }
            ...
            if (count($set) > 0) {
                $set = implode(', ', $set);
                $sql = "UPDATE graphics SET $set WHERE id=?";
    ...
    $actions['delete'] = function () {
        ...
        $id = $_REQUEST['id'];
        try {
            qi('DELETE FROM graphics WHERE id=?', [$id]);
    ...
    function get_agregate($group_id = '')
    {
        ...
        $sql = "SELECT 1 as stub  FROM (SELECT main_table.*  FROM graphics main_table) temp $srch $filter $where $group_where $order";
    ...
    function get_data($force_kill_pagination = false)
    {
        ...
        $debug = (isset($_REQUEST['alef_debug']) && $_REQUEST['alef_debug'] == 1);
        if ($pagination == 1) {
            $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM graphics main_table) temp $srch $filter $where $order LIMIT :start, :limit";
            ...
            $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM graphics main_table) temp $srch $filter $where $order";
            if (in_array($_SERVER['SERVER_NAME'], ['test-genesis.alef.im', 'devtest-genesis.alef.im', 'localhost']) || $debug) {
                if ($_REQUEST['action'] == '') {
                    echo "<!--SQL DATA {$sql} -->";
                }
    ...
    $finalPage = masterRender('Графики', $content, 1, 'genesis-body-presentation-table genesis-grouping-style-');
